...
class DirectPromptAgent:
    """Passes the user prompt directly to the LLM without a system prompt."""
...
class RAGKnowledgePromptAgent:
    ...
    def chunk_text(self, text):
        ...
        if len(text) <= self.chunk_size:
            chunk = {"chunk_id": 0, "text": text, "chunk_size": len(text)}
...
class EvaluationAgent:
    """Iteratively evaluates and refines a worker agent's response against defined criteria."""

    def __init__(self, openai_api_key, persona, evaluation_criteria, worker_agent, max_interactions):
        """Initialize with API key, persona, evaluation criteria, worker agent, and max iterations."""
        self.openai_api_key = openai_api_key
        self.persona = persona
    ...
    def evaluate(self, initial_prompt):
        """Iteratively refine the worker agent's response until it meets the evaluation criteria."""
        client = OpenAI(api_key=self.openai_api_key)
...
class RoutingAgent():
    """Routes user prompts to the most appropriate agent using cosine similarity of embeddings."""

    def __init__(self, openai_api_key, agents):
        """Initialize with API key and a list of agent route dictionaries."""
        self.openai_api_key = openai_api_key
        self.agents = agents

    def get_embedding(self, text):
        """Return the embedding vector for the given text using text-embedding-3-large."""
        client = OpenAI(api_key=self.openai_api_key)
        response = client.embeddings.create(
            model="text-embedding-3-large",
            input=text,
    ...
    def route(self, user_input):
        """Select the best agent by cosine similarity and return its response."""
...
class ActionPlanningAgent:
    """Extracts actionable steps from a user prompt using provided domain knowledge."""

    def __init__(self, openai_api_key, knowledge):
        """Initialize with API key and knowledge string used to guide step extraction."""
        self.openai_api_key = openai_api_key
        self.knowledge = knowledge

    def extract_steps_from_prompt(self, prompt):
        """Return a clean list of action steps extracted from the given prompt."""
        client = OpenAI(api_key=self.openai_api_key)
        response = client.chat.completions.create(
            model="gpt-3.5-turbo",
            messages=[
